...
p. Sending test messages through the pdtest outlet.  Specifically used inside a _function()_ passed to the *case.test()* function.  These messages are _registered_ as test messages, pdtest will expect a test result for each of these test messages.  General rule: 1 *Test* call for each *case.test()*.
