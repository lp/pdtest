...
h4. Sending test messages through the pdtest outlet.  Specifically used inside a _function()_ passed to the *pd.test()* function.  These messages are _registered_ as test messages, pdtest will expect a test result for each of these test messages.  General rule: 1 *pdtest.out* call for each *pdtest.test()*.
...
h4. Sending raw messages through the pdtest outlet.  These _unregistered_ messages go unnoticed through the test system. Meant to be used inside _setup()/teardown()_ or as test helpers inside *pdtest.test()* _function()_.  General rule: 1 ignored inlet message for each *pdtest.raw* call.
