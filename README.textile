...
mycase.test(function()                                  -- long test form, function body can contain only one
    Test.list({"MY","TEST","MESSAGE"})            -- Test.<type>() call, but unlimited _.outlet.<type>().
end)
...
p. Sending test messages through the pdtest outlet.  Specifically used inside a _function()_ passed to the *pd.test()* function.  These messages are _registered_ as test messages, pdtest will expect a test result for each of these test messages.  General rule: 1 *Test* call for each *suite.test()*.

bc. Test.list({"YOUR","MESSAGE","LIST"})              -- sends lua table as test message list through outlet
Test.symbol("STRING")                             -- sends lua string as test message symbol through outlet
Test.number(3.1416)                               -- sends lua number as test message float through outlet
