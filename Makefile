...
$(LUAD)/build.stamp: $(LUAD)/unpack.stamp
	make -C $(LUAD) ansi local
	touch $(LUAD)/build.stamp

$(LUAD)/unpack.stamp: $(LUATGZ)
	tar xzf $(LUATGZ)
	touch $(LUAD)/unpack.stamp
	mv $(LUAD)/src/Makefile $(LUAD)/src/Makefile.old
	sed 's/^CFLAGS=.*$\/& $(LUACFLAGS)/' $(LUAD)/src/Makefile.old >$(LUAD)/src/Makefile.temp
	sed 's/^MYLDFLAGS=.*$\/& $(LUACFLAGS)/' $(LUAD)/src/Makefile.temp >$(LUAD)/src/Makefile
